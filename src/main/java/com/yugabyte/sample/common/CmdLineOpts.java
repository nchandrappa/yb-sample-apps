...
//    SqlHasura.class,
//    SqlHasuraJson.class,
//    SqlHasuraIndex.class,
    SqlUserPrivacyData.class,
    SqlProductUserOrdersUpdate.class
  );
  ...
  public void initialize(CommandLine commandLine) throws ClassNotFoundException {
	...
	if (commandLine.hasOption("data_load_prefix")) {
		AppBase.appConfig.loadStartSequence = Long.parseLong(commandLine.getOptionValue("data_load_prefix"));
	}
	LOG.info(String.format("SqlHasura: Data load primary key will start with prefix:  %d",
			AppBase.appConfig.loadStartSequence));

	if (commandLine.hasOption("action_type")) {
		AppBase.appConfig.actionType = commandLine.getOptionValue("action_type");
	}
	LOG.info(String.format("SqlHasura: Action type used for the dataload:  %s", AppBase.appConfig.actionType));

	
	if (commandLine.hasOption("json_column_size")) {
		AppBase.appConfig.jsonColumnSize = Integer.parseInt(commandLine.getOptionValue("json_column_size"));
	}
	LOG.info(String.format("SqlHasuraJson: No. of columns in json format:  %d", AppBase.appConfig.jsonColumnSize));
  ...
  public static CmdLineOpts createFromArgs(String[] args) throws Exception {
    ...
    options.addOption("data_load_prefix", true,
            "[SqlHasura] start prefix number for the data load.");
    
    options.addOption("action_type", true,
            "[SqlHasura] action type for the data load. It can have one of following values: "
            + "loadprimary, loadforeign");
    
    options.addOption("json_column_size", true, 
    		"[SqlHasuraJson] no of columns to be converted into json string");
