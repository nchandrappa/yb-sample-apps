 ...
 */
public class SqlProductUserOrdersUpdate extends AppBase {
  private static final Logger LOG = Logger.getLogger(SqlProductUserOrdersUpdate.class);
  ...
  private static final String DEFAULT_TABLE2_NAME = "user_orders";
  ...
  private static final String DEFAULT_TABLE2_COLUMNS = "userID,orderId,orderTotal,orderDetails,orderTime";
...
//The shared prepared update statement for Table 2.
 private volatile PreparedStatement preparedStatementForUpdateTable2 = null;
  ...
  private static Faker orderDataGenerator;

  public SqlProductUserOrdersUpdate() {
    buffer = new byte[appConfig.valueSize];
    fairy = Fairy.create();
    orderDataGenerator = new Faker();
  }
...
//      		              String.format("SELECT * FROM %s WHERE %s.userid = ?", getTable1Name(), getTable1Name()));
  ...
  private PreparedStatement getPreparedStatementForUpdateTable2() throws Exception {
	    if (preparedStatementForUpdateTable2 == null) {
	      synchronized (prepareInitLock) {
	        if (preparedStatementForUpdateTable2 == null) {   	
	          StringBuilder sb = new StringBuilder();
	          sb.append("UPDATE ").append(getTable2Name()).append(" ");
	          sb.append("SET orderTotal = ? ");
	          sb.append("WHERE userID = ?");
	          preparedStatementForUpdateTable2 = getPostgresConnection().prepareStatement(sb.toString());
	          LOG.info("Update prepared statement: " + sb.toString());
	        }
	      }
	    }
	    return preparedStatementForUpdateTable2;
	  }
  ...
  public long doWrite(int threadIdx) {
    ...
    } else if (appConfig.actionType.equalsIgnoreCase(AppConfig.ACTION_TYPES.loadforeign.toString())) {
      return insertDataTable2();
    } else {
    	return updateDataTable2();
    }
  }
...
//				statement.executeBatch();
  ...
  private Key preparedStatementMapper( PreparedStatement statement) throws SQLException {
      ...
      Long primaryKeyValue = key.asNumber();
	  ...
	  HashSet<Key> keys = new HashSet<>();
		...
		try {
			...
			if (appConfig.batchSize > 0) {
				...
				Key key = getSimpleLoadGenerator().getKeyToWrite();
				keys.add(preparedStatementMapperTable2(statement, key));
				result = statement.executeUpdate();
			}
			for (Key key : keys) {
				getSimpleLoadGenerator().recordWriteSuccess(key);
  ...
  private long updateDataTable2() {
	  HashSet<Key> keys = new HashSet<>();
		int result = 0;
		try {
			PreparedStatement statement = getPreparedStatementForUpdateTable2();
			if (appConfig.batchSize > 0) {
				for (int i = 0; i < appConfig.batchSize; i++) {
				...
				Key key = getSimpleLoadGenerator().getKeyToWrite();
				keys.add(preparedStatementMapperUpdateTable2(statement, key));
				result = statement.executeUpdate();
			}
			for (Key key : keys) {
				getSimpleLoadGenerator().recordWriteSuccess(key);
  ...
  private Key preparedStatementMapperTable2( PreparedStatement statement, Key key) throws SQLException {
	  
      Timestamp timeStamp = new Timestamp(System.currentTimeMillis());
      UUID uuid = UUID.randomUUID();
      String randomUUIDString = uuid.toString();
      
      statement.setLong(1, key.asNumber());
      statement.setString(2, randomUUIDString);
      statement.setString(3, orderDataGenerator.commerce().price());
      statement.setString(4, orderDataGenerator.commerce().productName());
      statement.setTimestamp(5, timeStamp);
      
      return key;
  }
  
private Key preparedStatementMapperUpdateTable2( PreparedStatement statement, Key key) throws SQLException {
	  
//      Timestamp timeStamp = new Timestamp(System.currentTimeMillis());
      Long primaryKeyValue = key.asNumber();
      
      statement.setString(1, orderDataGenerator.commerce().price());
//      statement.setTimestamp(2, timeStamp);
      statement.setLong(2, primaryKeyValue);
